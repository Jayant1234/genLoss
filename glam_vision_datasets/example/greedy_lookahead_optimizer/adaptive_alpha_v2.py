...
# Import our new adaptive optimizer
import numpy as np
...
class AdaptiveLookahead(torch.optim.Optimizer):
    ...
    def sigmoid(self, x):
        """Sigmoid function to map input to (0,1) range"""
        return 1 / (1 + np.exp(-x))

    def update_alpha(self, val_loss):
        """Update alpha based on validation loss metrics"""
        ...
        # Higher value means current performance is closer to best performance
        if self.running_val_loss > 0 and self.best_val_loss > 0:
            improvement_ratio = self.best_val_loss / self.running_val_loss
            
            # Scale improvement_ratio to be centered around 0
            # Values > 0 mean running_val_loss is better than best_val_loss
            # Values < 0 mean running_val_loss is worse than best_val_loss
            scaled_improvement = 5 * (improvement_ratio - 1)
